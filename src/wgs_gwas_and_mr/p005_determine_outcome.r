...
# load data
dx_data= data.table::fread("./data/output/D005DX.csv")
dx_data %>% names()
dx_data %>%
...
# left join with the tidy women data
D001TidyWomen= data.table::fread("./data/output/D001TidyWomen.csv")
D001TidyWomen %>% dim()
D006EligibleCasesandControls= outcome_data %>%
  left_join(
    D001TidyWomen,
    by = "eid"
  ) %>%
  select(eid, contains("eid"), everything())
D006EligibleCasesandControls %>% dim()
D006EligibleCasesandControls %>% names()


D006EligibleCasesandControls %>%
  fwrite(
    "./data/output/D006EligibleCasesandControls.csv",
    row.names = FALSE
  )
D006EligibleCasesandControls %>%
  select(eid) %>%
