...
mr_results_new[get(col_name) == "Yes", length(unique(`Assay Target`))]
# mr_results_new[get(col_name) == "Yes", ] %>%
#   dplyr::distinct() %>% nrow()
...
sig_metabolites %>% dim()
cols_subset = c("Assay Target", "Sample size", "OR", "OR.confint.lower", "OR.confint.upper", "Odds ratio (95% CI)", col_name, "P value", "Raw P value")
sig_metabolites = sig_metabolites[, .SD, .SDcols = cols_subset] %>% unique()
sig_metabolites %>% dim()
# sig_metabolites[, .(`Variant ID (CHROM:GENPOS (hg38):A0:A1)`)] %>%
#   data.table::fwrite(
#     "./output/p020_cumu_risk.txt",
#     col.names = FALSE
#   )
sig_metabolites %>% str()
sig_metabolites[, `:=`(
...
# p_left_data = sig_metabolites[, .(`Assay Target`, `Sample size`, Cases, Controls)]
p_left_data = sig_metabolites[, .(`Assay Target`, `Sample size`)]
p_left_data %>% head()
...
mr_forest_plot = omixVizR::plot_forest(p_left_data = p_left_data,
                                       ...
                                       p_left_data_name = c('Proteins', 'Sample size'),
