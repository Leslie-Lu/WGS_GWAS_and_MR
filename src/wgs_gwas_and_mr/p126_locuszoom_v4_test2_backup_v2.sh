...
outputdir="/share/home/lsy_luzhen/WGS_GWAS_and_MR/tmp_ssh_data/GWAS_european/step2/locuszoom_output_v4_v2"
mkdir -p "$outputdir"
mkdir -p "$outputdir/ld_files"
mkdir -p "$outputdir/figures_test2"
...
#   fi
  ...
  if /share/home/lsy_luzhen/software/locuszoom/locuszoom/bin/locuszoom \
        ...
        --pvalcol P \
        --refsnp "$rsid" \
        --denote-markers-file "$markers_file/markers_file_lead_snps_${chr}_${from_bp}_${to_bp}_20260124_v2.txt" \
        --start "$from_bp" \
        --end "$to_bp" \
        --chr "$chr" \
        --flank 500kb \
        ...
        --prefix "$outputdir/figures_test2/locuszoom_${refsnp}"; then
    echo "✓ Successfully processed $refsnp"
  else
    echo "✗ Failed to run locuszoom for $refsnp" >&2
  fi
done << EOF
chr6:32632785,chr6:32632785,6,32632785
EOF
